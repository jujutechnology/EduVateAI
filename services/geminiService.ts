...
import { FeedbackCategory, Message, MindmapNode, Quiz, ImmersiveText, WritingFeedbackResponse, Subject } from '../types';
...
export const generateCuriousSuggestion = async (recentSubjects: Subject[]): Promise<Subject | null> => {
    if (recentSubjects.length === 0) return null;

    const recentSubjectNames = recentSubjects.map(s => s.name).join(', ');
    const allSubjectNames = SUBJECTS.map(s => s.name);
    
    const systemInstruction = `You are a curriculum advisor. Based on the user's recently studied topics, suggest a new, related topic for them to explore from the provided list. The suggested topic MUST NOT be in their recent list. Respond with a single JSON object containing the exact "name" of the suggested topic. Example: {"suggestion": "World Geography"}`;

    const prompt = `
      All available topics: [${allSubjectNames.join(', ')}]
      User's recent topics: [${recentSubjectNames}]
      
      Suggest a new, related topic from the "All available topics" list that is NOT in the "User's recent topics" list.
    `;
    ...
    try {
        ...
        const suggestedName = jsonResponse.suggestion;
        
        return SUBJECTS.find(s => s.name === suggestedName) || null;
    } catch (error) {
        console.error("Error generating curious suggestion:", error);
        // Don't throw, just return null so the UI doesn't break
        return null;
    }
};
